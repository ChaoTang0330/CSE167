...
//glm::vec3  Window::eye, Window::center, Window::up; // replaced by camera
...
bool Window::initializeObjects()
{
	...
	sphere2World = new Transform(glm::mat4(1), 0, 0, 0, camera);
	sky2World = new Transform(glm::mat4(1), 0, 0, 0, camera);
	track2World = new Transform(glm::mat4(1), 0, 0, 0, camera);

	skyBox = new SkyBox(20.0f, programSky, camera);// , camera);
	sphere = new Sphere("sphere.obj", program, camera);
	track = new BeizerTrack(program,camera);

	world->addChild(sky2World);
	world->addChild(sphere2World);
	world->addChild(track2World);

	sky2World->addChild(skyBox);
	sphere2World->addChild(sphere);
	track2World->addChild(track);

	sphere->setTextureID(skyBox->getTextureID());

	track->setObject(sphere2World);
	...
	for (float i = -22.5; i <= 22.5; i = i + 5)


	{
		for (float j = -22.5; j <= 22.5; j = j + 5)
		{
			glm::mat4 T = glm::translate(glm::vec3(i,0,j));
			Transform* tempRot2Army = new Transform(T, 0, 1, 3.5);
			tempRot2Army->addChild(robot);
			tempRot2Army->addChild(sphere2Army);
			Army->addChild(tempRot2Army);
		}
	}
