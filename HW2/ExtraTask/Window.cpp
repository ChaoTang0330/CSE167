...
GLuint Window::lightDirectionLoc, Window::lightCutoffLoc, Window::lightExponentLoc;
GLuint Window::dlightPositionLoc, Window::dlightAmbientLoc;
GLuint Window::dlightDiffuseLoc, Window::dlightSpecularLoc, Window::dlightAttenuationParaLoc;
GLuint Window::dlightDirectionLoc, Window::dlightCutoffLoc, Window::dlightExponentLoc;
GLuint Window::lightSwitch(1), Window::lightSwitchLoc;

Material Window::material;
Light Window::light, Window::dlight;
...
GLuint Window::lightObjSwitchLoc;
...
bool Window::initializeProgram() {
	...
	lightPositionLoc = glGetUniformLocation(program, "spotLight.position");
	lightAmbientLoc = glGetUniformLocation(program, "spotLight.ambient");
	lightDiffuseLoc = glGetUniformLocation(program, "spotLight.diffuse");
	lightSpecularLoc = glGetUniformLocation(program, "spotLight.specular");
	lightAttenuationParaLoc = glGetUniformLocation(program, "spotLight.attenuationPara");
	lightDirectionLoc = glGetUniformLocation(program, "spotLight.direction");
	lightCutoffLoc = glGetUniformLocation(program, "spotLight.cutoff");
	lightExponentLoc = glGetUniformLocation(program, "spotLight.exponent");

	dlightPositionLoc = glGetUniformLocation(program, "directionLight.position");
	dlightAmbientLoc = glGetUniformLocation(program, "directionLight.ambient");
	dlightDiffuseLoc = glGetUniformLocation(program, "directionLight.diffuse");
	dlightSpecularLoc = glGetUniformLocation(program, "directionLight.specular");
	dlightAttenuationParaLoc = glGetUniformLocation(program, "directionLight.attenuationPara");
	dlightDirectionLoc = glGetUniformLocation(program, "directionLight.direction");
	dlightCutoffLoc = glGetUniformLocation(program, "directionLight.cutoff");
	dlightExponentLoc = glGetUniformLocation(program, "directionLight.exponent");

	lightSwitchLoc = glGetUniformLocation(program, "lightSwitch");
	...
	light.position = glm::vec3(0.0, 0.0, 10.0);
	...
	dlight.position = glm::vec3(0.0, 10.0, 0.0);
	dlight.ambient = glm::vec3(0.6, 0.2, 0.2);
	dlight.diffuse = glm::vec3(0.9, 0.5, 0.5);
	dlight.specular = glm::vec3(1.0, 0.6, 0.6);
	dlight.attenuationPara = 0.2;
	dlight.direction = glm::vec3(0.0, 0.0, 0.0);
	dlight.cutoff = 0.0;
	dlight.exponent = 0.5;
	...
	lightObjSwitchLoc = glGetUniformLocation(programLight, "lightSwitch");
	glUseProgram(program);
...
{	
	...
	glUniform1f(lightExponentLoc, light.exponent);

	glUniform3fv(dlightPositionLoc, 1, glm::value_ptr(dlight.position));
	glUniform3fv(dlightAmbientLoc, 1, glm::value_ptr(dlight.ambient));
	glUniform3fv(dlightDiffuseLoc, 1, glm::value_ptr(dlight.diffuse));
	glUniform3fv(dlightSpecularLoc, 1, glm::value_ptr(dlight.specular));
	glUniform1f(dlightAttenuationParaLoc, dlight.attenuationPara);
	glUniform3fv(dlightDirectionLoc, 1, glm::value_ptr(dlight.direction));
	glUniform1f(dlightCutoffLoc, dlight.cutoff);
	glUniform1f(dlightExponentLoc, dlight.cutoff);

	glUniform1i(lightSwitchLoc, lightSwitch);
	...
	glUniform1i(lightObjSwitchLoc, lightSwitch);
	lightObj->draw();
	glUseProgram(program);
...
void Window::scrollCallback(GLFWwindow* window, double xOffset, double yOffset)
{
	...
	if (lightRotateEnable == 1)
	{
		...
		if (lightSwitch == 2)
		{
			dlight.position = factor * dlight.position;
		}
...
void Window::cursorPositionCallback(GLFWwindow* window, double xPos, double yPos)
{
	...
	if (mouseLeftButtonFlag == 1)
	{
		...
		if (lightRotateEnable == 1)
		{
			if (lightSwitch == 2)
			{
				glm::vec4 lightPos = glm::vec4(dlight.position,1);
				GLfloat degree;
				glm::vec3 normalVec = glm::cross(cursorPrevPos, v);
				if (glm::length(normalVec) > 0)
				{
					normalVec = glm::normalize(normalVec);
					GLfloat cosdeg = glm::dot(cursorPrevPos, v);
					degree = glm::acos(cosdeg);
					lightPos = glm::rotate(degree, normalVec) * lightPos;
					dlight.position = glm::vec3(lightPos);
					//std::cout << "dlight pos" << dlight.position.x << dlight.position.y << dlight.position.z <<std::endl;
				}
			}
			if (lightSwitch == 1)
			{
				glm::vec3 curP = cursorPrevPos;
				glm::vec3 curV = v;
				curP.z = -curP.z;
				curV.z = -curV.z;

				glm::vec4 lightDir = glm::vec4(light.direction, 0);
				GLfloat degree;
				glm::vec3 normalVec = glm::cross(curP, curV);
				if (glm::length(normalVec) > 0)
				{
					normalVec = glm::normalize(normalVec);
					GLfloat cosdeg = glm::dot(curP, curV);
					degree = glm::acos(cosdeg);
					lightDir = glm::rotate(degree, normalVec) * lightDir;
					light.direction = glm::vec3(lightDir);
					light.direction.z = (light.direction.z > 0) ? 0 : light.direction.z;
				}
			}
		}
		cursorPrevPos = v;
